...
test("init auto-sets up Charon inside an Aeon repo", () => {
  const cwd = tmpdir();
  fs.mkdirSync(path.join(cwd, "skills", "demo"), { recursive: true });
  fs.mkdirSync(path.join(cwd, ".github", "workflows"), { recursive: true });
  fs.writeFileSync(path.join(cwd, "aeon.yml"), "demo: { enabled: true }\n");
...
test("aeon passport summarizes skill risk surfaces", () => {
  const cwd = tmpdir();
  fs.mkdirSync(path.join(cwd, "skills", "audit"), { recursive: true });
  fs.writeFileSync(path.join(cwd, "aeon.yml"), "audit: { enabled: true }\n");
...
test("aeon policy synth proposes reads writes network secrets and irreversible review", () => {
  const cwd = tmpdir();
